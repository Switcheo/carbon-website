...
const Ecosystem: React.FC = () => {
	...
	const [allBlockchains, setAllBlockchains] = React.useState<
		BlockchainConfig[]
	>([]);
	...
	const subTextContent: SubTextContent | null =
		subTextContentMap[dAppsFilter] ?? null;
					...
					>
						...
						<Grid item xs={12}>
							<Typography color="textSecondary" align="left">
								Carbon is powered by Cosmos SDK, builders, validators and
								community partners to create the future of DeFi.
							</Typography>
						</Grid>
						<Grid item xs={12}>
							<Banner
										...
										>
											<Button
												className={clsx(classes.filterButton, {
													active: dAppsFilter === filter,
												})}
												onClick={() => setDAppsFilter(filter)}
												disableRipple
											>
												{filter}
											</Button>
							...
							{subTextContent && (
								<>
									<Typography
										...
										color="textSecondary"
										align="left"
										style={{ maxWidth: "1024px", marginTop: "20px" }}
									>
								...
								{blockchainFilters.map((filter, index) => {
									return (
										...
										>
											<Button
												className={clsx(classes.filterButton, {
													active: blockchainFilter === filter,
												})}
												onClick={() => setBlockchainFilter(filter)}
												disableRipple
											>
												{filter} Chains
											</Button>
...
const useStyles = makeStyles((theme: Theme) => ({
	...
	contentBox: {
		...
		overflowY: "auto",
		"--mask-image-content":
			"linear-gradient(to bottom, transparent, #010103 var(--mask-height), #010103 calc(100% - var(--mask-height)), transparent)",
