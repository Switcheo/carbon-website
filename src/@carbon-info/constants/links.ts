export const Path = {
	Header: {
		Build: "https://docs.carbon.network",
		SWTH: "https://hub.carbon.network/",
	},
	Feedback: "https://ideas.dem.exchange/",
	Socials: {
		LinkedIn: "https://www.linkedin.com/company/switcheo/",
		Twitter: "https://twitter.com/0xcarbon",
		Telegram: "https://t.me/demex_ann",
		Youtube: "https://www.youtube.com/channel/UCqZ9sxvw-0UoHzno4-d97oQ",
		Medium: "https://medium.com/switcheo",
		Discord: "https://discord.gg/SPh62Hf",
		Demex: "https://app.dem.exchange",
		Github: "https://github.com/Switcheo/carbon-bootstrap",
		SwitcheoForum:
			"https://commonwealth.im/carbon-protocol/discussions/Proposal%20Discussion",
		CoinGecko:"https://www.coingecko.com/en/coins/carbon-protocol",
	},
	Docs: {
		CarbonEVM: "https://docs.carbon.network/carbon-evm/overview",
		CarbonCore: "https://docs.carbon.network/carbon-core/overview",
	},
	Ecosystem: {
		Grant:"https://docs.carbon.network/grants-programs/carbon-ecosystem-grant",
	},
	Footer: {
		Features: "https://docs.carbon.network/carbon-core",
		Guides: "https://docs.carbon.network/introduction",
		Buy: "https://hub.carbon.network/buy",
		CarbonHub: "https://hub.carbon.network/",
		Stake: "https://hub.carbon.network/stake",
		Nitron: "https://app.dem.exchange/nitron",
		Token: "https://scan.carbon.network/tokens?net=main",
		Markets: "https://scan.carbon.network/markets?net=main",
		Demex: "https://dem.exchange",
		CarbonScan: "https://scan.carbon.network",
		Bridge:
			"https://app.squidrouter.com/?chains=1%2Ccarbon-1&tokens=0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48%2Cibc%2F7C0807A56073C4A27B0DE1C21BA3EB75DF75FD763F4AD37BC159917FC01145F0",
		Hydrogen: "https://hydrogen.carbon.network",
		SwitcheoDevelopmentFund:
			"https://guide.carbon.network/resources/carbon-development-fund",
		APIs: "https://docs.carbon.network/api-references",
		Github: "https://github.com/Switcheo/carbon-bootstrap/releases",
		SDK: "https://github.com/Switcheo/carbon-js-sdk",
		Forum: "https://commonwealth.im/carbon-protocol",
		Governance: "https://scan.carbon.network/governance",
		Setup:
			"https://docs.carbon.network/introduction/getting-started/carbon-node-setup",
		Research: "https://blog.switcheo.com/tag/research/",
		BrandAsset:
			"https://www.notion.so/Carbon-Media-Kit-819ec0d0ed7e45389da48ffd3dbfa6a2",
		Partnerships: "/partnerships",
	},
	FindOutMore: "https://hub.carbon.network/",
};
